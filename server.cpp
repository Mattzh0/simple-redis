...
// read a 4-byte integer from 'cur' into 'out'
// *& means a reference to a pointer, as we want to modify the pointer itself and not a copy
...
static bool read_str(const uint8_t *&cur, const uint8_t *end, size_t n, std::string &out) {
	...
	out.assign(cur, cur + n); // clears out, copies bytes starting at cur till (not including) cur + n, resizes out to exactly n bytes
	cur += n;
	return true;
}
...
static int32_t parse_req(const uint8_t *data, size_t size, std::vector<std::string> &out) {
	...
	// populate the num_strings data
	if (!read_u32(data, end, num_strings)) {
		return -1;
	}

	// make sure the number of strings is within the safety limit 'k_max_args'
	if (num_strings > k_max_args) {
		return -1;
	}

	while (out.size() < num_strings) {
		uint32_t len = 0;
		...
		out.push_back(std::string()); // create new empty string at the end of the vector as a "bucket" to be filled by call to read_str()
...
// hash table placeholder, to be implemented later
...
static void do_request(std::vector<std::string> &cmd, Response &out) {
	if (cmd.size() == 2 && cmd[0] == "get") {
		...
		const std::string &val = it->second; // get the value in the key-value pair
